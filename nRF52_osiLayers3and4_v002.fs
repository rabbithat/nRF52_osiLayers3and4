...
\ only display on the terminal node.  Do not display on the remote node
: errorMsg_link <bool_terminalNode?> @ if <bool_terminalNode?> @ if CR ." ***Error in radio link!***  No acknowledgment from target device." CR then then ;
